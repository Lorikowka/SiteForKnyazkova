...
app.post('/api/create-payment',
  ...
  async (req, res) => {
    ...
    try {
      ...
      // Проверяем, что платёж создан успешно и есть confirmation_url
      if (!payment.confirmation || !payment.confirmation.confirmation_url) {
        logger.error(`❌ ЮKassa не вернула confirmation_url:`, JSON.stringify(payment, null, 2));
        throw new Error('ЮKassa не вернула ссылку для оплаты. Ответ: ' + JSON.stringify(payment));
      }
